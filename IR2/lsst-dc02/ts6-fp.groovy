...
taskConfig = ["monitor-update/taskPeriodMillis":3000,"monitor-publish/taskPeriodMillis":9000,
              "agentStatusAggregatorService/patternConfigList":[
              "[pattern:.*,predicate:[agentName:${focalPlaneName}],alias:focal-plane]",
              "[pattern:.*,predicate:[agentName:fp-rebps],alias:rebps]"]
             ]

def partition = props.getProperty("org.lsst.ccs.raft.partition","ts6")
        ...
        for (Reb rebGeometry : raftGeometry.getChildrenList() ) {
            ...
            "$reb" (REBDevice, hdwType:"daq2", id:rebCount, ifcName:partition, ccdMask:7, ccdType:"ITL") {
                ...
                fitsService (FitsService, 
                   headerFilesList:["primary", "extended", "ts6-fp-reb_cond", "ts6-fp-test_cond"]
                )
